...
class SprintBurndownDashboard(AbstractDashboard):
    ...
    def multi_prepare(self, data_spent, data_original):
        ...
        fl_original = float(sum([orig for orig, st in zip(data_original['timeoriginalestimate'], data_original['status'])
                                 if st not in ('Closed', 'Resolved') and data_original['flagged']]))
        for i in range(len(data_spent['key'])):
            if data_spent['flagged']:
                fl_spent += float(data_spent['spent'][i])
                self.fl_all_spent[data_spent['created'][i]] = spent
            spent += float(data_spent['spent'][i])
            self.all_spent[data_spent['created'][i]] = spent
        for resdate, origest in zip(data_original['resolutiondate'], data_original['timeoriginalestimate']):
            if resdate is not None:
                if data_original['flagged']:
                    fl_original += float(origest)
                    fl_all_original[resdate] = original
                original += float(origest)
                all_original[resdate] = original
        self.all_remain = {dt: all_original.get(dt, original) - self.all_spent[dt] for dt in self.all_spent.keys()}
        self.fl_all_remain = {dt: fl_all_original.get(dt, original) - self.fl_all_spent[dt]
    ...
    def export_to_plotly(self):
        ...
        layout = go.Layout(
            ...
            yaxis1=dict(
                ...
                range=[0, math.ceil(max(self.all_remain.values())/100)*100]
            ),
            xaxis2=dict(
                domain=[0, 1],
                anchor='y2',
            ...
            yaxis2=dict(
                ...
                range=[0, math.ceil(max(self.fl_all_remain.values()) / 100) * 100]
            ),
            title=title,
            legend=dict(
                orientation='h',
