WITH user_stories AS (
	SELECT us_ji.ID AS 'ID', (CASE WHEN us_pp.ID IS NOT NULL THEN 'Pilot 2.0' ELSE NULL END) AS 'Pilot 2.0', (CASE WHEN us_core.ID IS NOT NULL THEN 'Core' ELSE NULL END) AS 'Core',
	...
	FROM [srv-jira-prod-report].[dbo].[jiraissue] AS us_ji
		...
		LEFT JOIN (SELECT pp_ji.ID FROM [srv-jira-prod-report].[dbo].[label] AS pp_lbl
			INNER JOIN [srv-jira-prod-report].[dbo].[jiraissue] AS pp_ji ON pp_ji.ID = pp_lbl.ISSUE WHERE pp_lbl.LABEL = 'Pilot2.0') AS us_pp ON us_pp.ID = us_ji.ID
	...
	WHERE us_prj.pname = 'BSSBOX' AND us_it.pname = 'User Story (L3)' AND us_is.pname != 'Canceled' AND us_lbl.LABEL = 'Pilot2.0'),
epics AS (
	SELECT e_ji.ID AS 'ID', (CASE WHEN epics_cmp.component IS NULL THEN '' ELSE epics_cmp.component END) AS 'component',
		user_stories.[Pilot 2.0] AS 'Pilot 2.0', user_stories.Core AS 'Core', user_stories.[Custom] AS 'Custom', user_stories.Config AS 'Config'
...
tasks AS (
	SELECT t_ji.ID AS 'ID', epics.component AS 'component',
		epics.[Pilot 2.0] AS 'Pilot 2.0', epics.Core AS 'Core', epics.[Custom] AS 'Custom', epics.Config AS 'Config'
...
subtasks AS (
	SELECT st_ji.ID AS 'ID', tasks.component AS 'component',
		tasks.[Pilot 2.0] AS 'Pilot 2.0', tasks.Core AS 'Core', tasks.[Custom] AS 'Custom', tasks.Config AS 'Config'
...
SELECT CONCAT_WS('-', prj.pkey, ji.issuenum) AS 'key', issues.component AS 'component', CONVERT(date, wl.STARTDATE) AS 'created', CONVERT(date, ji.RESOLUTIONDATE) AS 'resolutiondate', wl.timeworked / 28800 AS 'spent', ist.pname AS 'status',
	issues.[Pilot 2.0] AS 'Pilot 2.0', issues.Core AS 'Core', issues.[Custom] AS 'Custom', issues.Config AS 'Config'
