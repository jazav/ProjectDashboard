WITH user_stories AS (
	SELECT us_ji.ID AS 'id', CONCAT_WS('-', us_prj.pkey, us_ji.issuenum) AS 'key', us_it.pname AS 'issue type', us_is.pname AS 'status', NULL AS 'resolution date',
		...
		(CASE WHEN us_pp.ID IS NOT NULL THEN 'Pilot 2.0' ELSE NULL END) AS 'Pilot 2.0', (CASE WHEN us_core.ID IS NOT NULL THEN 'Core' ELSE NULL END) AS 'Core',
...
epics AS (
	SELECT DISTINCT e_ji.ID AS 'id', CONCAT_WS('-', e_prj.pkey, e_ji.issuenum) AS 'key', e_it.pname AS 'issue type', e_is.pname AS 'status', CONVERT(date, e_ji.RESOLUTIONDATE) AS 'resolution date',
		epics_cmp.Component AS 'component', user_stories.[key] AS 'L3', NULL AS 'estimate', user_stories.[Pilot 2.0] AS 'Pilot 2.0', user_stories.Core AS 'Core', user_stories.[Custom] AS 'Custom', user_stories.Config AS 'Config'
...
SELECT id, [key], [issue type], [status], [resolution date], component, L3, CONCAT('{', STRING_AGG(estimate, ', '), '}') AS 'estimate', [Pilot 2.0], Core, [Custom], Config
FROM user_stories
GROUP BY id, [key], [issue type], [status], [resolution date], component, L3, [Pilot 2.0], Core, [Custom], Config
UNION ALL
SELECT * FROM epics
