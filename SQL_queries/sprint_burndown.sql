...
SELECT issues.[Pilot 1.0] AS 'Pilot 1.0', issues.[Pilot 2.0] AS 'Pilot 2.0', CONCAT_WS('-', prj.pkey, ji.issuenum) AS 'key', issues.component AS 'component', CONVERT(date, wl.STARTDATE) AS 'created',
    CONVERT(date, ji.RESOLUTIONDATE) AS 'resolutiondate', wl.timeworked / 28800 AS 'spent', ist.pname AS 'status'
